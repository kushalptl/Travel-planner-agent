...
async function checkHealth() {
  try {
    ...
    const ok   = data.model_ready;
    DOM.statusDot.className  = `status-dot ${ok ? 'online' : 'offline'}`;
    DOM.statusText.textContent = ok ? `Online · ${data.model}` : 'Model not configured';
  } catch {
...
function injectWelcome() {
  const msg = `## 👋 Hello! I'm **TravelAI**, your IBM Watsonx.ai travel companion.
